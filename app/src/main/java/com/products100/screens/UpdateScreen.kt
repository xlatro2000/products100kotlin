...
import com.products100.constants.APPTitlesAndDesc.TITLE_UPDATE
import com.products100.model.BookModel
import com.products100.viewmodel.ViewModelInsertBook
import com.products100.viewmodel.ViewModelUpdateBook
...
fun UpdateScreen (
    ...
    NavToBack: () -> Unit,
    viewModel: ViewModelUpdateBook = hiltViewModel()
){
    val context = LocalContext.current
    val bookItem by viewModel.bookUpdated.collectAsState(BookModel(bookname = "", bookauthor = "", bookimage = ""))
    ...
    Column(
        ...
        Button(
            onClick = {
                ...
                //Log.d("INICIO ACTUALIZACION", "Se inicia la actualizacion")

                viewModel.updateBook(book)

                //Log.d("FIN ACTUALIZACION", "Se Finaliza la actualizacion")
                Toast.makeText(context, book.toString(), Toast.LENGTH_SHORT).show()
            },
            modifier = Modifier.background(color = MaterialTheme.colorScheme.secondary)
        ) {
            Text(APPTitlesAndDesc.TITLE_UPDATE)
        }
    }
}